...
function validation(req, res, next) {//middlreware validator for review
    if(req.body.id && req.body.title){
        if(!books.some(elem => elem.id === req.body.id)){
            if(req.body.reviews && req.body.reviews.length > 0){
                if(Array.isArray(req.body.reviews)){
                    req.body.reviews.forEach(reviews => {
                        if((!reviews.id || !reviews.comment) || (reviews.id == '' || reviews.comment == '')){
                            next(createError(400, 'Every review must have both fields \'id\' and \'comment\' filled'))
                            return
                        }
                    });
            ...
            next(createError(400, `There is already exist book with such id! You can choose id which is bigger than ${books.length-1}`))
            return
        }
    }else{
...
router.get('/:id/reviews/:reviewId', (req, res, next)=>{
    if(books[req.params.id].reviews.some(el => el.id == req.params.reviewId)){
        ...
        next(createError(400, `There isn't rview with such id: ${req.params.reviewId}`))
        return
    }
})

router.post('/', validation, (req, res)=>{ // add some author with posts
    books.push({id: req.body.id,title: req.body.title, reviews: req.body.reviews});
    res.status(200).json({books});
})

router.put('/:id/reviews', (req, res)=>{ // add some new review
...
router.delete('/:id/reviews/:reviewId', (req, res, next)=>{ //remove some review
    if(books.some(book => book.id === req.params.id)){
        if(books[req.params.id].reviews.some(review => review.id === req.params.reviewId)){
            books[req.params.id].reviews = books[req.params.id].reviews.filter(review => review.id !== req.params.reviewId)
            res.json(books)
        }else{
            next(createError(400, `There isn't such review with id: ${req.params.reviewId}`))
            return
        }
    }else{
        next(createError(400, `There isn't such book with id: ${req.params.id}`))
